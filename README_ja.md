...
- XLSX の共有文字列はファイル内の順序と重複を維持して読み取るため、同じ文字列が複数登録されたファイルも変換可能
- ARB 形式への出力（キーはアルファベット順にソート）
- CLI とライブラリの両方で利用可能

XLSX の読み取りはローカライズ表に必要なセル値に限定しています。数式は計算せず式文字列を読み取ります（共有数式の従属セルは保存されている値を返します）。表示形式は日付・時刻の判別にのみ使うため、桁区切りや通貨記号、条件付き書式は再現しません。マクロ、画像、グラフ、結合セルの展開は扱いません。
...
- CLI オプション:
  ...
  - `--treat-undefined-placeholders`: `warn|ignore|add|error` のいずれかを指定します。検出されたプレースホルダがシート内で宣言されていない場合の振る舞いを制御します。効果を持たせるには `--auto-detect-placeholders` が必要です。振る舞い:
    ...
    - `error`: 未定義プレースホルダを検出した時点で終了コード `65`（`EX_DATAERR`）で中断します。
